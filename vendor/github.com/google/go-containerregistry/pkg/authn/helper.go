...
func (h *helper) queryCommand(helperName, in string, out bytes.Buffer) (string, error) {
	...
	cmd.Stdout = &out
	cmdErr := h.r.Run(cmd)
	return strings.TrimSpace(out.String()), cmdErr
}
...
func (h *helper) Authorization() (string, error) {
	...
	// Try new behavior as implemented by current Docker first
	if key == "index.docker.io" {
		key = "https://index.docker.io/v1/"
	}

	var out bytes.Buffer
	output, cmdErr := h.queryCommand(helperName, key, out)
	// If we see this specific message, it means the domain wasn't found
	// and we may fall back on anonymous auth.
	if output == magicNotFoundMessage {
		// Fall back on legacy behavior of helper next
		out.Reset()
		// Some keychains expect a scheme:
		// https://github.com/bazelbuild/rules_docker/issues/111
		output, _ = h.queryCommand(helperName, fmt.Sprintf("https://%v", h.domain), out)
		if output == magicNotFoundMessage {
			// Best effort - fall back on anonymous auth
